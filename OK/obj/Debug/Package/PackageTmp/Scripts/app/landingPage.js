var element = (function () {
    var canCircle = function () {
       ...
       //globe img element
        im = new Image();
        im.src = '../../images/globe.png';
        im.onload = function () {
            //ctx.elementImage(im, 100, 100, 100, 100);
        ...
        function draw() {
            ...
            var time = new Date();
            ////ctx.rotate(((2 * Math.PI) / 190) * time.getSeconds() + ((2 * Math.PI) / 60000) * time.getMilliseconds());
            //ctx.translate(1, 0);
            //ctx.drawImage(im,10, 50, 25,25);
        ...
        if (positon.x < 100 && positon.x > 40 && positon.y > 149 && positon.y < 230)
        {
            $('#pop-up').dialog({
                ...
                buttons: [{click:cv,class:'leftButton',width:50,height:50},
                    { width: 50, height: 60, class: 'rightButton', click: function () { document.location.href = '/Content/cv/CV.pdf' } }],
                modal: true,
